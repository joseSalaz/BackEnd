...
namespace Bussines
{
  public class LibroBussines : ILibroBussines
  {
    ...
    public LibroBussines(IMapper mapper, IFirebaseStorageService firebaseStorageService, IPrecioRepository iPrecioRepository, IKardexRepository kardexRepository
        ...
        ILibroAutorRepository libroAutorRepository, IUnitOfWork unitOfWork,
        IAzureComputerVisionService visionService)
    {
    ...
    public async Task<List<LibroCatalogo>> GetLibrosCatalogoByIds(List<int> ids)
    {
      ...
      var libros = await _ILibroRepository
          .GetLibrosConPreciosByIds(ids);



      return libros.Select(x => new LibroCatalogo
      {

        Libro = x,

        Precio = x.Precios
              .FirstOrDefault()?.PrecioVenta ?? 0
    ...
    public async Task<List<LibroFiltroResponse>> FiltrarLibros(FiltroLibroRequest request)
    {
    ...
    public async Task<Microsoft.Azure.CognitiveServices.Vision.ComputerVision.Models.ImageAnalysis> AnalizarImagenAsync(IFormFile archivo)
    {
      ...
      {
        throw new ArgumentException("No se proporcionó un archivo de imagen válido.", nameof(archivo));
      }
      using var stream = archivo.OpenReadStream();
      return await _visionService.AnalyzeImageAsync(stream);
    }
