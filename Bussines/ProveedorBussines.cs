...
namespace Bussines
{
  public class ProveedorBussines : IProveedorBussines
  {
    ...
    #region constructor 
    public ProveedorBussines(IMapper mapper, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<List<ProveedorResponse>> getProveedorCategoria(int idCategoria, int? idSubcategoria)
    {
      List<Proveedor> pr = await _IProveedorRepository.getProveedorCategoria(idCategoria, idSubcategoria);
      List<ProveedorResponse> res = _Mapper.Map<List<ProveedorResponse>>(pr);
      return res;
    }
  }
    #endregion
