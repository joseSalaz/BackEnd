...
namespace Bussines
{
  public class SubcategoriaBussines : ISubcategoriaBussines
  {
    ...
    #region constructor 
    public SubcategoriaBussines(IMapper mapper, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<List<LibroCatalogo>> GetLibrosCatalogoBySubcategoria(int idSubcategoria)
    {
      var ids = await _ISubcategoriaRepository
          .GetLibroIdsBySubcategoria(idSubcategoria);
      var libros = await _unitOfWork.Libros.GetLibrosConPreciosByIds(ids);
      var response = libros.Select(x => new LibroCatalogo
      {
