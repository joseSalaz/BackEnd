...
namespace Repository
{
  public class LibroRepository : GenericRepository<Libro>, ILibroRepository
  {
    ...
    public async Task<List<LibroFiltroResponse>> FiltrarLibros(
        FiltroLibroRequest request)
    {
      string? autores = null;
      ...
      {
        proveedores = string.Join(",", request.Proveedores);
      }

      var result = await db.LibroFiltroResponse
          .FromSqlInterpolated($@"
            EXEC SP_FiltrarLibros
    ...
    public async Task<IEnumerable<LibroCatalogoDTO>> ObtenerCatalogoOptimizadoAsync()
    {
      return await dbSet
          ...
          .Select(l => new LibroCatalogoDTO
          {
            ...
            // SOLUCIÓN 1: Manejo del decimal nullable
            PrecioVenta = l.Precios.Select(p => p.PrecioVenta).FirstOrDefault() ?? 0m,

            Autor = l.LibroAutors
                       .Select(la => la.IdAutorNavigation.Nombre)
                       .FirstOrDefault() ?? "Desconocido",

            // SOLUCIÓN 2: Cambia "Descripcion" por la propiedad real que tengas en Subcategoria
            Categoria = l.IdSubcategoriaNavigation.Descripcion ?? ""
          })
          .ToListAsync();
    }
    ...
    public async Task<List<Libro>> GetLibrosConPreciosByIds(List<int> ids)
    {
      ...
      return await db.Libros
          .Where(x => ids.Contains(x.IdLibro))
          .Include(x => x.Precios)
          .ToListAsync();
